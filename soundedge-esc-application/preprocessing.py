...
def load_audio_with_soundfile(file_path: str):
    ...
    waveform = torch.tensor(waveform, dtype=torch.float32).transpose(
        0, 1
    )  # [channels, samples]
    return waveform, sr
...
def resample_if_needed(
    waveform: torch.Tensor, orig_sr: int, target_sr: int = TARGET_SR
) -> torch.Tensor:
...
def pad_or_trim(
    waveform: torch.Tensor, target_num_samples: int = TARGET_NUM_SAMPLES
) -> torch.Tensor:
