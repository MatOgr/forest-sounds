...
def plot_spectrogram(spec, sr=SR, hop=512, ax=None, y_axis="mel", title=""):
    ...
    img = librosa.display.specshow(
        spec, sr=sr, hop_length=hop, x_axis="time", y_axis=y_axis, ax=ax
    )
    ax.set_title(title)
...
def plot_features(y, sr=SR, title=""):
    ...
    librosa.display.specshow(mf, x_axis="time", ax=ax[1, 1])
...
def plot_confusion(
    y_true, y_pred, class_names=None, normalize=True, ax=None, title="Confusion matrix"
):
